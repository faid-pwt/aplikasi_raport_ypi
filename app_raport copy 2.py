...
def generate_excel(data_siswa, df_nilai, data_tambahan):
    ...
    ws.merge_cells('A1:F1'); ws['A1'] = "MADRASAH DINIYAH ULA"; ws['A1'].font = font_kop_1; ws['A1'].alignment = align_center
    ...
    ws.merge_cells('A9:B9'); ws['A9'] = "No. Statistik"
    ...
    rt += 2
    ws.merge_cells(f'A{rt}:F{rt}'); ws[f'A{rt}'] = "ABDULLAH MUNIR, S.Pd"; ws[f'A{rt}'].font = font_bold_u; ws[f'A{rt}'].alignment = align_center
...
# --- WATERMARK EXCEL TRANSPARANSI TINGKAT TINGGI ---
    try:
        ...
        # 2. Proses Pemisahan Warna (KUNCI TRANSPARANSI TERBAIK)
        for item in datas:
            # Jika piksel aslinya sudah transparan, biarkan transparan
            if item[3] == 0:
                newData.append((255, 255, 255, 0))
            # Jika piksel berwarna putih atau abu-abu sangat terang (latar belakang)
            elif item[0] > 220 and item[1] > 220 and item[2] > 220:
                newData.append((255, 255, 255, 0)) # Buat jadi 100% BOLONG
            # Jika ini adalah warna logonya (Hijau/Kuning)
            else:
                # Angka 25 ini adalah tingkat transparansi (10% terlihat)
                # Garis tabel tidak akan tertutup karena latar belakangnya sudah bolong di atas
                newData.append((item[0], item[1], item[2], 16))
